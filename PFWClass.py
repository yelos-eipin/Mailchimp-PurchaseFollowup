...
class PFW:
    ...
    def get_equipment_purchases(self, days_prior):
        ...
        sql = """
                    ...
                    AND cust.cu_cus NOT LIKE 'JDWAR%'
                    AND cust.cu_cus NOT LIKE 'TRAN%'   
                    AND cust.cu_cus NOT LIKE 'INVADJ%'
                    AND mh.bh_ord <> 'LATECH' -- Exclude Late charge invoices
                    AND md.bd_tc = 'FC'-- For equipment, only look at Final Charge to the customer
                    AND mh.bh_bc IN ('I') -- choose Equipment billing types
                    AND contact.cc_bem <> ''
                    AND cust.cu_cus NOT IN (select 
                                                cust.cu_cus "Customer Number"
                                                FROM 
    ...
    def get_service_performed(self, days_prior):
        ...
        sql = """
                    ...
                    AND cust.cu_cus NOT LIKE 'JDWAR%'
                    AND cust.cu_cus NOT LIKE 'TRAN%'
                    AND md.bd_tc = 'FC' -- Look only at customer's Final Charge (includes tax)
                    AND mh.bh_bc IN ('W') -- choose Service Work Order invoice types
                    AND contact.cc_bem <> '' -- Leave out those without an email
                    AND cust.cu_cus NOT IN (SELECT 
                                                cust.cu_cus "Customer Number"
                                                FROM 
                                                mnbdh mh
